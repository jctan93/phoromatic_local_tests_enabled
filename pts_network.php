...
class pts_network
{
	...
	private static $network_timeout = 5;
	...
	public static function http_get_contents($url, $override_proxy = false, $override_proxy_port = false, $override_proxy_user = false, $override_proxy_pw = false, $http_timeout = 5)
	{
		...
		$contents = pts_file_io::file_get_contents($url, 0, $stream_context) ? pts_file_io::file_get_contents($url, 0, $stream_context) : pts_file_io::file_get_contents($url, 0, $ctx);
	...
	public static function stream_download($download, $download_to, $stream_context_parameters = null, $callback_function = array('pts_network', 'stream_status_callback'))
	{
		...
		*/
		...
		$file_pointer = @fopen($download, 'r', false, $stream_context) ? @fopen($download, 'r', false, $stream_context) : @fopen($download, 'r', false, $ctx) ;
	...
	public static function client_startup()
	{
		...
		self::$network_timeout = pts_config::read_user_config('PhoronixTestSuite/Options/Networking/Timeout', 5);
