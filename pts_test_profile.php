...
class pts_test_profile extends pts_test_profile_parser
{
	...
	public static function is_test_profile($identifier)
	{
		...
		//Extra code here, code works on the assumption that all files exist or not at all. Eg. if test-definition.xml exists, 
		//then all other files are there. If it doesn't then all files don't exist
		if(!is_file(PTS_TEST_PROFILE_PATH . $identifier . '/test-definition.xml'))
                {
			if (strpos($identifier, 'local/') !== false) 
			{
				...
				if($servers_found)
				{
				//Gets the test name and also the name of the client/user to determine the location of the test profile folder
				$extracted_test_name_pos  = strpos($identifier, "/");
				$extracted_test_name  = substr($identifier, $extracted_test_name_pos + 1);
				$current_user  = exec('whoami');
				//echo '-----Name to search for in the download-cache.php data: ' . $extracted_test_name . '-----' .PHP_EOL;
				$dire_name;
				
				//Can't create folder without sudo access :(
				if($current_user === 'root')
				{
				}
				else
				{
				//	/home/jctan93/.phoronix-test-suite/test-profiles/local
					$dire_name = '/home/'. $current_user  .'/.phoronix-test-suite/test-profiles/local/' . $extracted_test_name;
					//echo '-----Directory name: ' . $dire_name . '-----' . PHP_EOL;
					
					//Checks if the directory exists and creates it otherwise 
					if(!is_dir($dire_name))
					{
						mkdir($dire_name, 0777, true);
						echo '-----Creating Directory ' . $dire_name . '-----' . PHP_EOL;
					}
					else
						echo '-----Directory' . $dire_name  .  ' exists-----' . PHP_EOL;
				}

				foreach($archived_servers as $archived_server)
				{
					$url = 'http://' . $archived_server['ip'] . ':' . $archived_server['http_port'] . '/download-cache.php';
					//echo '-----$url is ' . $url . '-----' .PHP_EOL;
					//$repo = pts_network::http_get_contents('http://' . $archived_server['ip'] . ':' . $archived_server['http_port'] . '/download-cache.php');
					$repo = pts_network::http_get_contents($url);
					$split_repo = explode("\n", $repo);
					$test_file_name;
					foreach($split_repo as $link_line)
					{
						$is_test_link = strpos($link_line, $extracted_test_name."/");
						if($is_test_link)
						{
							$matches;
							//$matches[0][0] is the name of the file/s to download eg mytest/install.sh
							preg_match('/'. $extracted_test_name .'\/[A-Za-z0-9-]*.[A-Za-z0-9]*/', $link_line, $matches, PREG_OFFSET_CAPTURE);
							$url = 'http://' . $archived_server['ip'] . ':' . $archived_server['http_port'] . '/download-cache.php?download=' . $matches[0][0];
							$actual_test_name_pos = strpos($matches[0][0], '/');
							$actual_test_name  = substr($matches[0][0], $actual_test_name_pos);							

							echo '-----Downloading '. $matches[0][0]  . ' from the server to : ' . $dire_name . $actual_test_name . '-----' . PHP_EOL;
							pts_network::download_file($url, $dire_name . $actual_test_name);
						}
					}
