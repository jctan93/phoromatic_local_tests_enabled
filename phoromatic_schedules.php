		...
		if(!empty($PATH[0]) && is_numeric($PATH[0]))
		{
			...
			else
			{
				...
				if(!PHOROMATIC_USER_IS_VIEWER)
				{
					...
					$root_test_profiles_dir = '/var/lib/phoronix-test-suite/test-profiles/local';
					
					if(is_dir($root_test_profiles_dir))
					{
						$folders_in_dir = scandir($root_test_profiles_dir);
						for($i = 2; $i < sizeof($folders_in_dir); $i++)
						{
							$main .= '<option value="local/' . $folders_in_dir[$i]  . '">' . 'local/'  . $folders_in_dir[$i]  . '</option>';
						}
					}					
